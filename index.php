                ...
                <div class="p-5">
                  ...
                  <form class="user" method="post" action="">
                    <div class="form-group">
                      <input type="email" class="form-control form-control-user" name="mail" aria-describedby="emailHelp" placeholder="Enter Email Address...">
                    </div>
                    <div class="form-group">
                    ...
                    <input type="submit" class="btn btn-primary btn-user btn-block" name="conn">
                    <?php
                    session_start();
                    $connexion = new PDO('mysql:host=localhost;dbname=hopital','sakho','1234');
                        ...
                        {
                          extract($_POST);
                         $traitement = $connexion->prepare("SELECT * FROM secretaire WHERE  login = '$mail' and pwd ='$pwd'");
                          $resultat = $traitement->execute();
                          if($resultat)
                          {
                            $affiche=$traitement->fetchAll();
                            foreach ($affiche as $affiche);
                            $_SESSION['id']=$affiche['id'];
                            $_SESSION['nom']=$affiche['nom'];
                             $_SESSION['prenom']=$affiche['prenom'];
                             $_SESSION['src']=$affiche['src'];
                            if ($affiche['login']==$mail and $affiche['pwd']==$pwd)
                              {
                              ...
                              {
                                echo "<font color='red'>nom ou mot de passe incorrecte </font>";
                              }
                          }
